...
# Direct inference endpoint — bypasses the auto-router entirely
ENDPOINT   = f"https://api-inference.huggingface.co/models/{MODEL_ID}/v1"
MAX_TOKENS = 1024
...
def _get_client() -> AsyncInferenceClient:
    ...
    if _client is None:
        ...
        # Pass the base_url directly so no auto-router is involved
        _client = AsyncInferenceClient(base_url=ENDPOINT, token=token)
    return _client
